...
def count_words(phrase):
    ...
    phrase = phrase.split() #listified


    for each in phrase:

        words[each] = words.get(each, 0) + 1 #word becomes key
...
def print_melon_at_price(price):
    ...
    for melon, cost in sorted(melon_prices.dict.items()):
        if price == cost:
            print(melon)
        else:
            print("None found")
...
def kids_game(names):
    ...
    for name in names:#iterate names
        ...
        if name[0] not in words: # if not in dict (idx)

            words[name[0]] = []#push it
